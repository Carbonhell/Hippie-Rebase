...
/datum/objective/New(var/text, datum/mind/mind)
	..()
...
/datum/objective/proc/requirements()
	return 1
				...
				if(player.mind.special_role && player.mind != owner)
					if(player.stat != DEAD)
						if(istype(player, /mob/living/silicon))
							continue
						if(player.onCentcom())
							if(!istype(get_turf(player), /turf/open/floor/plasteel/shuttle/red) && !istype(location, /turf/open/floor/mineral/plastitanium/brig))
								...
								if(O.telecrystals < U.telecrystals) // Checks to see if anybody has more TC than you and is also on centcom
									leader = 0
		if(leader == 1 && location.onCentcom() && !istype(location, /turf/open/floor/plasteel/shuttle/red) && !istype(location, /turf/open/floor/mineral/plastitanium/brig)) // If you have more TC than anybody else and get to centcom, you greentext, otherwise it checks to see if you have the disk.
			return 1
...
var/list/departmentPoints = list(/datum/objective/crew/research = 0, /datum/objective/crew/engineering = 0, /datum/objective/crew/medical = 0, /datum/objective/crew/security = 0, /datum/objective/crew/supply = 0, /datum/objective/crew/service = 0, /datum/objective/crew/civilian = 0)
...
/datum/objective/crew/New(text, mind)
	..()

	update_explanation_text()

	if(tracked) trackedCrewObjectives |= src

/datum/objective/crew/check_completion(win = FALSE)
	if(win)
		var/departmentPath

		for(var/i in departments)
			if(departments[i] == department) departmentPath = i

			departmentPoints[departmentPath]++

/datum/objective/crew/research
	department = "R&D"
	jobs = "Research Director#Scientist#Roboticist"

/datum/objective/crew/research/techLevel
	jobs = "Research Director#Scientist"

	var/datum/tech/targetTech
	var/targetLevel

/datum/objective/crew/research/techLevel/New(text, mind)
	targetTech = pick(typesof(/datum/tech) - /datum/tech)

	targetLevel = rand(3, 6)

	..()

/datum/objective/crew/research/techLevel/check_completion()
	for(var/obj/machinery/r_n_d/server/server in machines)
		if(server.disabled) continue

		var/datum/tech/serverTech = locate(targetTech) in server.files.known_tech
		if(serverTech.level >= targetLevel)
			..(TRUE)
...
/datum/objective/crew/research/techLevel/update_explanation_text()
	..()

	explanation_text = "Reach level [targetLevel] in [initial(targetTech.name)]."
...
/datum/objective/crew/research/getSlime/New(text, mind)
	..()

	slimeColor = pick("gold", "adamantine", "pink", "oil", "black")

/datum/objective/crew/research/getSlime/check_completion()
	for(var/mob/living/simple_animal/slime/slime in mob_list)
		if(slime.colour == slimeColor)
			..(TRUE)
...
/datum/objective/crew/research/getSlime/update_explanation_text()
	...
	explanation_text = "Produce at least one [slimeColor] slime."

/datum/objective/crew/research/fixTelescience
	jobs = "Research Director#Scientist"

/datum/objective/crew/research/fixTelescience/check_completion()
	var/obj/machinery/computer/telescience/computer = locate() in machines
	if(computer && computer.telepad)
		..(TRUE)
...
/datum/objective/crew/research/telesci/update_explanation_text()
	...
	explanation_text = "Build a Telescience computer, and link it to a Telepad."

/* Will do later, bot tracking isn't in the rebase and it's more trouble than it's worth at the moment
/datum/objective/crew/research/buildBots
	jobs = "Research Director#Roboticist"

	var/obj/machinery/bot/targetType
	var/targetAmount = 0

/datum/objective/crew/research/buildBots/New(text, mind)
	..()

	targetType = pick(typesof(/mob/living/simple_animal/bot) - /mob/living/simple_animal/bot/mulebot)
	targetAmount = rand(2,4)

/datum/objective/crew/research/buildBots/check_completion()
	var/list/craftedBots = SSbot.processing - SSbot.roundstartbots

	var/currentAmount = 0
	for(var/obj/machinery/bot/bot in craftedBots)
		if(istype(bot, targetType))
			currentAmount++
...
/datum/objective/crew/research/buildBots/update_explanation_text()
	..()

	explanation_text = "Craft at least [targetAmount] number of [initial(targetType.name)]s and ensure they survive till the shift ends. Roundstart ones don't count, so name them!"
*/

/datum/objective/crew/research/buildDrones
	jobs = "Research Director#Roboticist"
...
/datum/objective/crew/research/buildDrones/New(text, mind)
	..()

	targetAmount = rand(10,15)

/datum/objective/crew/research/buildDrones/check_completion()
	var/currentAmount = 0

	for(var/i in existingShells)
		currentAmount++

	for(var/mob/living/simple_animal/drone/drone in living_mob_list)
		currentAmount++
...
/datum/objective/crew/research/buildDrones/update_explanation_text()
	..()

	explanation_text = "Build at least [targetAmount] drones, doesn't matter if active or not, but they must be kept intact."

/datum/objective/crew/research/borgs
	jobs = "Research Director#Roboticist"
	var/amt

/datum/objective/crew/research/borgs/New(text, mind)
	..()
	amt = rand(2,3)
...
/datum/objective/crew/engineering/fullsmes/New(text, mind)
	..()
...
/datum/objective/crew/engineering/engines/check_completion()
	for(var/obj/singularity/sing in poi_list)
		if(sing.energy >= 500) // stage 3
			var/count = locate(/obj/machinery/field/containment) in urange(30, sing, 1)
			if(count)
				engines++
				break
	for(var/obj/machinery/power/supermatter_shard/sm in machines)
...
/datum/objective/crew/engineering/nofire/New(text, mind)
	..()
	alert = new()
...
/datum/objective/crew/engineering/airalarm/New(text, mind)
	..()
	check = new()
...
/datum/objective/crew/medical/survivor/New(text, mind)
	..()
	percneeded = rand(6,9)
...
/datum/objective/crew/medical/defib/New(text, mind)
	..()
...
/datum/objective/crew/medical/defib/check_completion()
	if(!owner)
		return
	var/list/fbdetails = detailsToList("medicalshit", " ", "|")
...
/datum/objective/crew/medical/power/New(text, mind)
	..()
...
/datum/objective/crew/medical/clone/New(text, mind)
	..()
...
/datum/objective/crew/medical/makechem/New(text, mind)
	..()
...
/datum/objective/crew/medical/makechem/check_completion()
	var/list/fbdetails = detailsToList("chemical_reaction", " ", "|")
...
/datum/objective/crew/medical/havechem/New(text, mind)
	..()
...
/datum/objective/crew/medical/fridgesymp/New(text, mind)
	..()
...
/datum/objective/crew/security/gunsinarmory/New(text, mind)
	..()
	egunamt = rand(1,3)
	laseramt = rand(1,3)
	shotgunamt = rand(1,4)
...
/datum/objective/crew/security/arrestantag/check_completion()
	...
	for(var/area/A in world)
		if(istype(A, /area/shuttle/escape))
			E = A
			break
	for(var/turf/open/floor/plasteel/shuttle/red/R in area_contents(E))
...
/datum/objective/crew/security/buddy/requirements()
	...
	if(target && target.current && owner && owner.current)//should exist by now
		var/datum/objective/crew/security/buddy/B = new(mind = target)
		B.target = owner
...
/datum/objective/crew/supply/points/New(text, mind)
	..()
...
/datum/objective/crew/supply/miningpoints/New(text, mind)
	..()
...
/datum/objective/crew/supply/mineore/New(text, mind)
	..()
	var/divider = 1
...
/datum/objective/crew/supply/mineore/check_completion()
	var/list/details = detailsToList("ore_mined", " ", "|")
...
/datum/objective/crew/service/bartender/New(text, mind)
	..()
	amt = rand(50,100)
	...
	drink = initial(drinkpath.results)
...
/datum/objective/crew/service/collectplants/New(text, mind)
	..()
	amt = rand(70,100)
	var/loop = rand(3,5)
...
/datum/objective/crew/service/clean/New(text, mind)
	..()
...
/datum/objective/crew/civilian/slip/New(text, mind)
	..()
	amt = rand(20,40)

/datum/objective/crew/civilian/slip/check_completion()
	var/list/dat = detailsToList("slips", " ", "|")
	for(var/i in dat)
...
/datum/objective/crew/civilian/reporter/New(text, mind)
	..()
	articles = rand(2,4)
...
/datum/objective/crew/civilian/petition/New(text, mind)
	..()
...
/datum/objective/crew/civilian/relic/New(text, mind)
	..()
	amt = rand(10,20)
...
/datum/objective/crew/civilian/nostun/check_completion()
	if(owner && owner.current)
		var/list/details = detailsToList("stuns", " ", "|")
